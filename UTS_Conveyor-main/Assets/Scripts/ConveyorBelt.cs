...
{
	public static ConveyorBelt instances;
    ...
    private void MoveItems()
    {
        ...
        {
			_items[i].item.GetComponent<BoxBehaviour>().index = i;
	...
	{
		GameObject itemToDes = _items[i].item.gameObject;
        _items.RemoveAt(i);
		StartCoroutine(DestroyEffect(itemToDes));
	}

	public ConveyorBeltItem GetItem(int i)
	{
		return _items[i];
	}

	private IEnumerator DestroyEffect(GameObject objToDes)
	{
		int c = 3;
		SpriteRenderer s = objToDes.GetComponent<SpriteRenderer>();
		s.color = Color.white;
